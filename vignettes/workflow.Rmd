...
- `draw_posterior`: Draws `nsim` samples from the posterior distribution 
...
```{r}
elong %<>% mutate(draws = map(survey, draw_posterior, nsim=10))
...
```{r}
elong %<>% mutate(seats = map2(draws, survey, get_seats, distrib.fun=sls2))
